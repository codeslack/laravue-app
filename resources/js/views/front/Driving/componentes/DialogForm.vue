<template>
    <v-dialog v-model="show" persistent max-width="500px">
        <v-card>
            <v-card-title>
                <span class="title">{{ formTitle }} {{ labeled }}</span>
            </v-card-title>
            <v-card-text>
                <v-container>
                    <v-row>
                                ...
                                >
                                    <template v-slot:activator="{ on }">
                                        <v-text-field
                                            ...
                                            v-on="on"
                                            required
                                            :error-messages="dateOfBirth_Errors"
                                            @change="
                                                $v.editedItem.date_of_birth.$touch()
                                            "
                                        />
                                    </template>
                                    <v-date-picker
...
import DialogTypeahead from "./../../../../components/typeahead/DlTypeahead";
import {
    required,
    email,
    numeric,
...
export default {
    ...
    computed: {
        ...
        formTitle() {
            return this.editedIndex === 0 ? "New" : "Edit";
        },
        toDatePicker: {
            get() {
    ...
    methods: {
        onServiceChanged(e) {
            ...
            Vue.set(this.$data.editedItem, "amount", customer.retailer);
            Vue.set(
        ...
        onSave() {
            ...
            if (!this.$v.$error) {
                if (this.editedIndex != 0) {
                    ...
                    this.method = "POST";
                    this.store = "/driving";
                    formData = objectToFormData(this.editedItem);
                }
