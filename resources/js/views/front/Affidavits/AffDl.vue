										...
										<v-card-text>
											<v-container>
												<v-row>
													...
													<v-col cols="12" sm="6" md="6">
														<v-text-field counter v-model="editedItem.mobile_no" label="Mobile No"></v-text-field>
													</v-col>
													<v-col cols="12" sm="12">
...
export default {
	...
	methods: {
		...
		printPdf(item) {
			...
			axios({
				url: "/driving-affidavits/print/" + item.identifier,
				method: "GET",
				responseType: "blob"
			})
				.then(response => {
					...
					// let newWindow = window.open("/loding");

					// newWindow.onload = () => {
					// 	newWindow.location = URL.createObjectURL(blob, "_blank");
					// };
