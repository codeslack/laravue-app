<template>
	<v-app>
		<v-app-bar app>
			...
			<v-spacer></v-spacer>
			<v-btn v-for="link in links" :key="link" text rounded class="d-none d-sm-flex my-2">{{ link }}</v-btn>
			<v-btn color="accent" @click.stop="showScheduleForm=true">hello</v-btn>
		</v-app-bar>
			...
			<v-layout column style="background:#FFFFFF">
				<v-flex xs12 style="background:#5277AC">
					<v-layout row class="mt-3">
						<v-flex xs2></v-flex>
						<v-avatar size="48">
							<span class="white--text headline">Home</span>
						</v-avatar>
						<v-spacer></v-spacer>
						<v-breadcrumbs class="white--text" :items="brdCrmbsItms" divider=">"></v-breadcrumbs>
						<v-flex xs2></v-flex>
					</v-layout>
				</v-flex>
...
export default {
	...
	data: () => ({
		...
		hoverStyle: "",
		show: false,
		links: ["Home", "About", "Team", "Contact", "Login"],
		icons: [
			{
				title: "Facebook",
				link: "#",
